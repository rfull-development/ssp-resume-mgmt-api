...
namespace ResumeManagementApi.Adapters
{
    public class UsageAdapter(IUsageDatabaseClient client)
    {
        ...
        public async Task<Usage?> GetAsync(string id)
        {
            ...
            Database.Models.Usage? dbItem = await _client.GetAsync(dbId);
            if (dbItem is null)
            {
                return null;
            }
        ...
        public async Task<bool> SetAsync(Usage usage)
        {
            ...
            Database.Models.Usage? dbUsage = await _client.GetAsync(dbId);
            if (dbUsage is null)
            {
                return false;
            }
        ...
        public async Task<List<Usage>> GetListAsync(string? startId, int limit)
        {
            ...
            List<Database.Models.Usage> dbUsages = await _client.GetListAsync(dbId, limit);
            List<Usage> usages = [];
            foreach (Database.Models.Usage dbUsage in dbUsages)
            {
