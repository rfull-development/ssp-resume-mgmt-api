...
namespace ResumeManagementApi.Adapters
{
    public class LearningAdapter(ILearningDatabaseClient client)
    {
        ...
        public async Task<Learning?> GetAsync(string id)
        {
            ...
            Database.Models.Learning? dbItem = await _client.GetAsync(dbId);
            if (dbItem is null)
            {
                return null;
            }
        ...
        public async Task<bool> SetAsync(Learning learning)
        {
            ...
            Database.Models.Learning? dbLearning = await _client.GetAsync(dbId);
        ...
        public async Task<List<Learning>> GetListAsync(string? startId, int limit)
        {
            ...
            List<Database.Models.Learning> dbLearnings = await _client.GetListAsync(dbId, limit);
            List<Learning> learnings = [];
            foreach (Database.Models.Learning dbLearning in dbLearnings)
            {
