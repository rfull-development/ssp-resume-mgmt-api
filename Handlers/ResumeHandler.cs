...
using ResumeManagementApi.Database.Clients.Postgres;
using ResumeManagementApi.Database.Exceptions;
...
namespace ResumeManagementApi.Handlers
{
    public static class ResumeHandler
    {
        public static void AddResumeHandler(this WebApplication app)
        {
            ...
            resumes.MapPatch("/{id}", UpdateResumeAsync);
        ...
        private static async Task<IResult> UpdateResumeAsync(
            [FromRoute(Name = "id")] string id,
            [FromBody] Models.ResumeUpdateRequest updateRequest
            )
        {
            bool success;
            try
            {
                ...
                using var databaseClient = databaseClientFactory.CreateResumeDatabaseClient();
                ResumeAdapter adapter = new(databaseClient);
                Models.Resume resume = new()
                {
                    Id = id
                };
                success = await adapter.SetAsync(resume);
            }
            catch (ArgumentException)
            {
                return Results.BadRequest();
